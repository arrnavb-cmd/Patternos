...
export default function MasterDashboard() {
  ...
  const fetchData = async () => {
    try {
      const [intentRes, commerceRes, platformRes, brandsRes, oppsRes] = await Promise.all([
        fetch('https://patternos-production.up.railway.app/api/master/intent-stats?clientId=zepto'),
        fetch('https://patternos-production.up.railway.app/api/master/dashboard-v2?clientId=zepto'),
        fetch('https://patternos-production.up.railway.app/api/master/platform-revenue?clientId=zepto'),
        fetch('https://patternos-production.up.railway.app/api/master/brand-performance-v2'),
        fetch('https://patternos-production.up.railway.app/api/master/revenue-opportunities?clientId=zepto&minScore=0.7')
      ]);
  ...
  const topBrands = commerceData?.brands
              ...
              {opportunities.map((opp, idx) => (
                <div key={idx} className="bg-slate-700/50 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-orange-400 font-bold text-sm">Score: {opp.avg_intent_score}</span>
                  </div>
                  <div className="space-y-2 mb-3">
                    <div className="text-sm text-slate-400">
                      <span className="text-white font-medium">{opp.users}</span> users • {formatCurrency(opp.potential_revenue)}
                    </div>
                  </div>
                  <div className="mb-3">
                    <p className="text-xs text-slate-500 mb-1">CONTACT BRANDS:</p>
                    <div className="flex flex-wrap gap-1">
                      {opp.brands.map(brand => (
                        <span key={brand} className="px-2 py-1 bg-slate-600 text-white text-xs rounded">{brand}</span>
                      ))}
                    </div>
                  </div>
                  <button 
              ...
              <tbody>
                {topBrands.map((brand, idx) => (
                  <tr key={idx} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                    ...
                    <td className="py-4 px-4 text-slate-300">{brand.purchases}</td>
                    <td className="py-4 px-4 text-blue-400">{brand.ctr}%</td>
                    <td className="py-4 px-4 text-purple-400">{brand.conv_rate}%</td>
                    <td className="py-4 px-4">
                      <div className="flex flex-wrap gap-1">
                        {Object.keys(brand.channels).slice(0, 3).map(channel => (
                          <span key={channel} className="px-2 py-1 bg-slate-700 text-xs text-slate-300 rounded">
                            {channel.replace('_', ' ')}
                          </span>
                        ))}
                      </div>
