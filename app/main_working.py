...
async def dashboard(clientId: str = "zepto"):
    try:
        ...
        return {"total_gmv": 0, "attributed_revenue": 0, "users_tracked": 0, "high_intent_users": 0, "platform_revenue": 0}
...
async def platform_rev(clientId: str = "zepto"):
    try:
        # Base platform fee: 3 Lakhs per month
        base_platform_fee = 300000
        
        # 10% commission on ad spend
        conn_business = sqlite3.connect("patternos.db")
        cursor_business = conn_business.cursor()
        cursor_business.execute("SELECT COALESCE(SUM(total_spent), 0) FROM campaigns WHERE is_active = 1")
        total_ad_spend = cursor_business.fetchone()[0]
        ad_commission = total_ad_spend * 0.10
        
        # 20% share of high-intent campaign revenue
        cursor_business.execute("SELECT COALESCE(SUM(total_revenue), 0) FROM campaigns WHERE is_active = 1")
        total_campaign_revenue = cursor_business.fetchone()[0]
        high_intent_premium = total_campaign_revenue * 0.20
        
        conn_business.close()
        
        # Monthly retainer (annual contract / 12)
        monthly_retainer = 416666.67  # 5M / 12
        
        total_revenue = base_platform_fee + monthly_retainer + ad_commission + high_intent_premium
        ...
        print(f"Error in platform_rev: {e}")
        return {
            "monthly_retainer": 416666.67,
            "ad_commission": 0,
            "high_intent_premium": 0,
            "total_revenue": 716666.67,
            "total_ad_spend": 0,
            "high_intent_campaign_spend": 0
        }
