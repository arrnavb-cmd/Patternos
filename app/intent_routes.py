...
router = APIRouter(tags=["Intent Intelligence"])
...
async def get_audience_users(
    ...
    query = """
        SELECT user_id, category, intent_score, signal_count, last_activity
        FROM intent_scores
        WHERE intent_score > 0.3
        ORDER BY intent_score DESC
        LIMIT ?
    """
    ...
    for row in cursor.fetchall():
        users.append({
            ...
            "intent_score": round(row[2] * 100, 1),
            "last_active": row[4] or "N/A",
            "avg_product_price": 0
        })
