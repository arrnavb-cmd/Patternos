...
export default function Analytics() {
  ...
  const fetchInteractiveData = async () => {
    ...
    try {
      ...
      if (compareBy === "location") {
        endpoint = `http://localhost:8000/api/v1/analytics/location-comparison?date_range=${dateRange}`;
      } else if (compareBy === "category") {
        endpoint = `http://localhost:8000/api/v1/analytics/category-comparison?date_range=${dateRange}`;
      }
  ...
  const fetchAnalyticsData = async () => {
    setLoading(true);
    try {
      let url = `http://localhost:8000/api/v1/analytics/platform-summary?date_range=${dateRange}`;
      ...
      if (activeTab === "brand") {
        const brandRes = await fetch(`http://localhost:8000/api/v1/analytics/brand-comparison?date_range=${dateRange}`);
